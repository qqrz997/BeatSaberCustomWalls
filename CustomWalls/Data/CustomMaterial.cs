...
public class CustomMaterial
{
    ...
    public static async Task<CustomMaterial> CreateAsync(string fileName)
    {
        ...
        try
        {
            AssetBundle assetBundle = await AssetBundleExtensions.LoadFromFileAsync(Path.Combine(Plugin.AssetPath, fileName));
            ...
            MeshFilter materialMeshFilter = MeshUtils.GetMeshFilterByName(gameObject, "pixie");
    ...
    public static async Task<CustomMaterial> CreateFromDataAsync(byte[] materialObject, string name)
    {
        ...
        {
            Plugin.Log.Warn($"Something went wrong getting the AssetBundle from resource!");
            Plugin.Log.Warn(ex);
    ...
    private static async Task RepairObjectShaders(GameObject gameObject)
    {
        ...
        if (!shaderReplacementInfo.AllShadersReplaced)
        {
            ...
            {
                Plugin.Log.Debug($"Failed to replace shader '{shaderName}' on {gameObject.name}");
            }
        }
    }
